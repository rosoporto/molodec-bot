...
def send_welcome(message: types.Message):
    ...
    bot.reply_to(
        message,
        "Привет! 👋 Я бот 'МОЛОДЕЦ'. Добавь привычку с /new_habit!"
    )
...
def save_habit(message: types.Message):
    ...
    if not habit:
        ...
        bot.reply_to(
            message,
            "Привычка не может быть пустой! Попробуй ещё раз с /new_habit"
        )
        return
    ...
    bot.reply_to(
        message,
        f"Привычка '{habit}' принята! Начни с /done, когда сделаешь первый шаг!"
    )
...
def done(message: types.Message):
    ...
    if data[user_id]["day"] == 0:
        ...
        bot.reply_to(
            message,
            f"Старт! '{data[user_id]['habit']}': М {progress_bar} (1/7). Ты молодец!"
        )
    else:
        # Проверка пропуска
        if last_date:
